<script setup>
import '../../../../../bricks/styles/_element-icon.scss';
...
import TheElementIndexRow from '../../../../components/m/element-switch/TheElementIndexRow.vue';
...
function doRefreshItems() {
    busy.add('modules.m.element-switch.index:fetch-items');
    api
        .request({
            method: 'GET',
            url: '/modules/m/element-switch/index',
        })
        .then(response => response.data)
        .then(data => {
            items.value = data.data.elements.map(item => {
        ...
        .finally(() => {
            busy.remove('modules.m.element-switch.index:fetch-items');
            resetBulkSelection();
        });
}
...
function doUpdateStatus(item, status = null) {
    ...
    busy.add('modules.m.element-switch.index:update-status');
    item.isUpdatingStatus = true;

    api
        .request({
            method: 'PATCH',
            url: `/modules/m/element-switch/update-status/${item.namespace}/${item.id}`,
            data: {
        ...
        .finally(() => {
            item.isUpdatingStatus = false;
            busy.remove('modules.m.element-switch.index:update-status');
        });
}
        ...
        <table class="wp-list-table widefat table-auto plugins">
            <thead>
                <tr>
                    ...
                    <th scope="col">
                        {{ __('Name', 'yabe-bricksbender') }}
                    </th>
                    <th scope="col">
                        {{ __('Category', 'yabe-bricksbender') }}
                    </th>
                </tr>
            </thead>
            <tbody v-if="items.length > 0 && !getBusyHasTask('modules.m.element-switch.index:fetch-items')">
                <TheElementIndexRow v-for="item in items" :key="item.id" :item="item" @update-status="doUpdateStatus(item, null)" />
            </tbody>
            <tbody v-else-if="getBusyHasTask('modules.m.element-switch.index:fetch-items')">
